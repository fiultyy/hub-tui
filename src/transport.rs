...
mod tests {
    ...
    #[test]
    fn test_orch_message_read_accepts_bool_and_int_and_null() {
        fn parse_read(json_val: &str) -> bool {
            let s = format!(r#"{{"id":"x","from_handle":"a","to_handle":"b","subject":"","body":"","type":"status","read":{json_val},"created_at":""}}"#);
            serde_json::from_str::<crate::model::OrchMessage>(&s).unwrap().read
        }
        assert!(!parse_read("0"));
        assert!(parse_read("1"));
        assert!(!parse_read("false"));
        assert!(parse_read("true"));
        assert!(!parse_read("null"));
    }
}
